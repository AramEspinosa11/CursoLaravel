...
class UtilesController extends Controller
{
    ...
    public function utiles_pdf()
    {
        ...
        $mpdf = new \Mpdf\Mpdf();
        $mpdf->WriteHTML('<h1>Mi primer reporte PDF desde Laravel!</h1><img src="https://www.tamila.cl/assets/img/logo_2.png" />');
        $mpdf->Output();
    }
    ...
    public function utiles_excel()
    {
        ...
        $spreadsheet->getProperties()->setCreator('Aram Espinosa')
            ->setLastModifiedBy('Aram.cl')
            ->setTitle('Office 2007 XLSX Test Document')
            ->setSubject('Office 2007 XLSX Test Document')
            ->setDescription('Excel creado con PHP.')
            ->setKeywords('office 2007 openxml php')
            ->setCategory('Test result file');
        // Para hacer la consulta es necesario importar el modelo productos
    ...
    {
        $response = Http::get('http://localhost/Projects/Laravel/ejemplo1/public/api/v1/productos') ;
        // formatear
        $datos = $response->object();
        // treaer el json integro
        $json = $response->body();
